...
export function generateMafiaHint(room: GameRoom): string {
  ...
  if (mafia.length === 1) {
    ...
    return `단서: X맨의 이름 첫 글자는 "${masked[0]}" 입니다. (${masked})`;
  }
  if (mafia.length >= 2) {
    return `단서: 현재 생존 X맨은 ${mafia.length}명입니다. ${GENERIC_HINTS[Math.floor(Math.random() * GENERIC_HINTS.length)]}`;
  }
